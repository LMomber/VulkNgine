...
void Renderer::Render()
{
	...
	uint32_t imageIndex;
	vkAcquireNextImageKHR(vkDevice, swapchain, UINT64_MAX, m_imageAvailableSemaphore, VK_NULL_HANDLE, &imageIndex);
	...
	if (vkQueueSubmit(m_pDevice->GetQueue(QueueType::GRAPHICS), 1, &submitInfo, m_inFlightFence) != VK_SUCCESS)
	{
...
void Renderer::CreateSyncObjects()
{
	...
	if (vkCreateSemaphore(m_pDevice->GetVkDevice(), &semaphoreInfo, nullptr, &m_imageAvailableSemaphore) != VK_SUCCESS ||
		vkCreateSemaphore(m_pDevice->GetVkDevice(), &semaphoreInfo, nullptr, &m_renderFinishedSemaphore) != VK_SUCCESS ||
		vkCreateFence(m_pDevice->GetVkDevice(), &fenceInfo, nullptr, &m_inFlightFence) != VK_SUCCESS)
	{
